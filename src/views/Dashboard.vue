<template>
  <div :v-if="isLoading">
    ...
    <div>
      <nav class="navbar fixed-top navbar bg-success">
        <center>
          ...
          <a class="navbar-brand ml-5" style="color:white" href="#">Dashboard</a>
        </center>
      </nav>
    </div>
    <!-- <h1>Dashboard</h1> -->
    ...
    <div id="chart" v-for="post in fakeArray" :key="post.id">
      <h3>{{ isLoading && post[0].facility.name }}</h3>
    ...
    userReports
      ...
      .then(() => {
        this.fakeArray.map((item, i) => {
          ...
          this.chartOptions.xaxis.categories.push(...m);
          anotherGraph[0].series[0].data.push(...val);
          anotherGraph[0].components = { apexchart: VueApexCharts };
          this.series[0].data.push(...val);
          ...
          // make.push({ components: { apexchart: VueApexCharts } });
